...
contract IndexRegistry is IndexRegistryStorage {
    ...
    /// @notice mapping from operatorId to address
    mapping(bytes32 => address) private operatorIdToAddress;
    ...
    function registerOperator(
        ...
        // Store the mapping
        operatorIdToAddress[operatorId] = msg.sender;
    ...
    /// @notice Returns the address associated with the given operatorId
    /// @dev Reverts if the operatorId does not exist
    function getOperatorFromId(
        bytes32 operatorId
    ) external view returns (address) {
        address operator = operatorIdToAddress[operatorId];
        require(operator != address(0), "IndexRegistry.getOperatorFromId: operator does not exist");
        return operator;
    }
}

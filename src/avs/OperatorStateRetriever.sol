...
contract OperatorStateRetriever {
        ...
        for (uint256 i = 0; i < quorumNumbers.length; i++) {
            ...
            for (uint256 j = 0; j < operatorIds.length; j++) {
                operators[i][j] = Operator({
                    operator: indexRegistry.getOperatorFromId(operatorIds[j]),
                    operatorId: operatorIds[j],
                    stake: stakeRegistry.getStakeAtBlockNumber(operatorIds[j], quorumNumber, blockNumber)
                });
            }
        }
     ...
     * @return indices struct containing all relevant indices for checkSignatures
     */
